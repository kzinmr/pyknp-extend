...
class Morpheme(object):
    ...
    def _parse_spec(self, spec):
        ...
        part = ''
        inside_quotes = False
        space_jumanpp = '      特殊 1 空白 6 * 0 * 0'
        space_juman = '  \\  \\  特殊 1 空白 6 * 0 * 0'
        if spec.startswith(space_juman):
            parts = ['\ ', '\ ', '\ ', '特殊', '1', '空白', '6', '*', '0', '*', '0', 'NIL']
        elif spec.startswith(space_jumanpp):
            parts = [' ', ' ', ' ', '特殊', '1', '空白', '6', '*', '0', '*', '0', 'NIL']
        else:
            for char in spec:
                if char == '"':
    ...
    def new_spec(self, prev_mrph_id=None, span=None):
        assert isinstance(prev_mrph_id, int) or isinstance(prev_mrph_id, str)\
             or isinstance(prev_mrph_id, list) or prev_mrph_id is None
        ...
        assert isinstance(span, tuple) or isinstance(span, list)\
            or isinstance(span, int) or isinstance(span, str) or span is None
        if span is None:
            span = self.span
        ...
        out.append("\t%s\t%s\t%s\t%s\t%s\t%s" % (self.bunrui, self.bunrui_id,\
                                                 self.katuyou1, self.katuyou1_id,\
                                                 self.katuyou2, self.katuyou2_id))
        out.append("\t")
        if not self.fstring:
            fs = []
