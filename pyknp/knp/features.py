...
class FeaturesTest(unittest.TestCase):
    ...
    def test(self):
        tag_str1 = "<BGH:構文/こうぶん><文節内><係:文節内><文頭><体言><名詞項候補><先行詞候補><正規化代表表記:構文/こうぶん>"
    ...
    def testPAS(self):
        pas_input = "分/ふん:判1:ガ/U/-/-/-/-;ヲ/U/-/-/-/-;ニ/U/-/-/-/-;デ/C/車/1/0/14;デ/C/徒歩/7/0/17;カラ/U/-/-/-/-;ヨリ/C/インター/0/0/14;マデ/U/-/-/-/-;ヘ/U/-/-/-/-;時間/U/-/-/-/-"
        tag_str2 = """<文末><カウンタ:分><時間><強時間><数量><体言><用言:判><体言止><レベル:C><区切:5-5><ID:（文末）><修飾><提題受:30><主節><状態述語><判定詞><正規化代表表記:３/さん+分/ふん><用言代表表記:分/ふん><時制-無時制><格関係0:ヨリ:インター><格関係1:デ:車><格解析結果:%s>""" % pas_input
        f2 = Features(tag_str2)
        self.assertEqual(f2.get("格解析結果"), pas_input)
        self.assertEqual(f2.pas.cfid, "分/ふん:判1")
        f2args = f2.pas.arguments
        self.assertEqual(len(f2args), 2)
        self.assertEqual(len(f2args.get("デ")), 2)
        self.assertEqual(f2args.get("デ")[0].rep, "車")
        self.assertEqual(f2args.get("デ")[0].tid, 1)
        self.assertEqual(f2args.get("デ")[0].sid, "14")
        self.assertEqual(f2args.get("デ")[1].rep, "徒歩")
        self.assertEqual(f2args.get("デ")[1].tid, 7)
        self.assertEqual(f2args.get("デ")[1].sid, "17")
        self.assertEqual(len(f2args.get("ヨリ")), 1)
        self.assertEqual(f2args.get("ガ"), None)
        self.assertEqual(f2.rels, None)

    def testRels(self):
        tag_str = """<rel type="時間" target="一九九五年" sid="950101003-002" id="1"/><rel type="ヲ" target="衆院" sid="950101003-002" id="3"/><rel type="ガ" target="不特定:人1"/><rel type="時間" target="国会前" sid="950101003-asd" id="16"/>"""
        f = Features(tag_str)
